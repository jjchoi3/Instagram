...
struct FollowButton: View {
    ...
    var body: some View {
        Button(action: follow) {
            HStack {
                Spacer()
                Text((self.isFollowing) ? "Unfollow" : "Follow").fontWeight(.bold).foregroundColor((self.isFollowing) ? Color.black : Color.white)
                Spacer()
            }.frame(height: 30).background((self.isFollowing) ? Color.white : Color.blue)
            
        }.cornerRadius(5).overlay(RoundedRectangle(cornerRadius:5).stroke(Color.gray.opacity(0.5), lineWidth: 1))
    }
}
